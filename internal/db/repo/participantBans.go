...
func (p *ParticipantBans) Add(participantId int, typeBan string, reason string, bannedAt time.Time, expiresAt time.Time) (int, error) {
	const sql = `INSERT INTO participant_bans (
		...
		RETURNING id`
	var id int
	err := p.Conn.QueryRowContext(context.Background(), sql, participantId, typeBan, reason, bannedAt, expiresAt).Scan(&id)
	if err != nil {
		return 0, fmt.Errorf("unable to ban participant (ID: %v) in database, %w", participantId, err)
	}
	return id, nil
}
func (p *ParticipantBans) Edit(id int, participantId int, typeBan string, reason string, bannedAt time.Time, expiresAt time.Time) error {
	const sql = `
		UPDATE participant_bans
		SET participant_id = $2, type_ban = $3, reason = $4, bannedAt = $5, expiresAt = $6
		WHERE id = $1`
	tag, err := p.Conn.ExecContext(context.Background(), sql, id, participantId, participantId, typeBan, reason, bannedAt, expiresAt)
	if err != nil {
...
func (p *ParticipantBans) Delete(id int) error {
	const sql = `
...
func (p *ParticipantBans) Get(id int) (entity.ParticipantBans, error) {
	const sql = `
...
func (p *ParticipantBans) IsBanned(participantId int) (bool, error) {
	const sql = `
		SELECT EXISTS(
	...
	var isBanned bool
	err := p.Conn.QueryRowContext(context.Background(), sql, participantId).Scan(&isBanned)
	return isBanned, err
}
