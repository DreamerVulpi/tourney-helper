...
import { X, Ban, ShieldCheck, Trash2, AlertTriangle } from 'lucide-react';
...
const ParticipantActionModal = ({ 
    ...
    participantData, // { nickname: 'DoshiPanda', ... }
    loading = false,
    theme = 'dark' 
}) => {
    ...
    const handleConfirm = () => {
        if (actionType === 'ban') {
            ...
            // Передаем собранные данные в onConfirm родительского компонента
            onConfirm({
                action: 'ban',
                id: participantData.id,
                typeBan: typeBan,            // Передаст сокращенный слаг (например, 'software/cheats')
                reason: reason.trim(),       // Передаст текст из textarea (ваше описание)
                ...
                <div className={`px-6 py-4 flex items-center justify-between border-b ${
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className={`text-sm font-black uppercase italic tracking-tight ${
                            ...
                            {config.title}: <span className="text-blue-500 not-italic normal-case ml-1">{participantData.nickname}</span>
                        </h2>
                    </div>
                    <button 
                        onClick={onClose}
