...
import { AddParticipant, GetParticipants, EditParticipant, EditParticipantStatsRating, AddBanToParticipant, DelBanFromParticipant, DelParticipant } from "../../wailsjs/go/application/App";
...
const DatabasePlate = ({ theme, statusDatabase }) => {
  ...
  // Имена платформ для бэкенда (можно вынести в стейты или пропсы, если они изменятся)
  ...
  const [selectedParticipantForAction, setSelectedParticipantForAction] = useState(null);
    ...
    try {
        if (data.action === 'ban') {
            const banRequest = {
                ...
                unit: data.unit, // 'minutes', 'hours', 'days', 'months'
          ...
          const unbanRequest = {
            participantId: selectedParticipantForAction.id
          };
            console.log(unbanRequest)
  ...
  const handleLocalRatingChange = (participantId, val) => {
    setPlayers(prev => prev.map(p => 
      (p.id === participantId) ? { ...p, rating: val } : p
    ));
  ...
  const fetchData = async (isNextPage = false, search = "") => {
    ...
    try {
      ...
      // Передаем динамические переменные платформ вместо строк хардкода
      const response = await GetParticipants(nameMessengerPlatform, nameTournamentPlatform, selectedGame, limit, currentOffset, currentSearch);
  ...
  const filteredPlayers = useMemo(() => {
    ...
    if (activeFilter === "banned") {
      list = list.filter(p => p.isBanned === "banned");
  ...
  const getRelativeTime = (dateString) => {
    if (!dateString || dateString.startsWith("0001")) return "—"; // Исключаем дефолтное время Go
            ...
            <div className="flex flex-col lg:flex-row items-center gap-3 w-full">
              ...
              >
                ...
                <div className={`absolute inset-0 flex gap-0.5 transition-all duration-300 z-20 ${isAddHovered ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"}`}>
                  ...
                  >
                    <UserPlus size={14} /> 
                    <span className="text-[7px] font-black uppercase mt-1 text-center">Одного игрока</span>
                  </button>
                  <div 
              ...
              <div className="relative group">
                ...
                <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none flex items-center">
                  {loading ? (
                    ...
                    <ChevronDown size={16} className={`transition-colors ${theme === 'dark' ? 'text-slate-600 group-hover:text-blue-500' : 'text-slate-400 group-hover:text-blue-600'}`} />
                  )}
                </div>
              </div>
                  ...
                  {filteredPlayers.map((p) => (
                    <tr key={p.id} className="hover:bg-blue-600/5 transition-colors align-middle">
                      <td className="p-4" style={{ width: `${sizeColumnOfNickname}px` }}>
                        <span className={`font-black text-[13px] italic tracking-tight break-all block ${
                      ...
                      {activeFilter === 'banned' ? (
                        <>
                          <td className="p-4 text-red-500 font-bold italic truncate" style={{ width: `${sizeColumnOfTypeBan}px` }}>
                            {p.typeBan || "Бан"}
                          </td>
                          ...
                          <td className="p-4 opacity-70 italic whitespace-nowrap" style={{ width: `${sizeColumnOfBannedAtDate}px` }}>
                          ...
                          <td className="p-4 font-bold whitespace-nowrap" style={{ width: `${sizeColumnOfExpiresDate}px` }}>
                            {p.expiresAt && !p.expiresAt.startsWith("0001") ? (
                              <span className="text-amber-500 opacity-80">{new Date(p.expiresAt).toLocaleDateString('ru-RU')}</span>
                            ) : (
  ...
  <div className="flex flex-col gap-1.5">
    ...
    {/* ДИНАМИЧЕСКАЯ КНОПКА БАНА / РАЗБАНА */}
    {p.isBanned === "banned" ? (
      <button 
        onClick={() => triggerParticipantAction(p, 'unban')} 
      ...
      <button 
        onClick={() => triggerParticipantAction(p, 'ban')} 
    ...
    <button onClick={() => triggerParticipantAction(p, 'delete')} className="w-full flex items-center justify-center gap-2 p-2 bg-red-600/10 hover:bg-red-600 text-red-500 hover:text-white rounded-lg text-[8px] font-black uppercase"><Trash2 size={11}/> Удалить </button>
  </div>
</td>
                        </>
                      )}
